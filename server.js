...
app.get("/api/stats", (req, res) => {
  ...
  db.get(`SELECT COUNT(*) as total, SUM(guest_count) as guests FROM reservations WHERE start_time LIKE ? AND status != 'cancelled'`, [`${today}%`], (err, row) => {
    res.json({ totalRes: row.total || 0, totalGuests: row.guests || 0, occupancy: Math.round(((row.total || 0) / tables.length) * 100) });
  });
});
...
app.post("/api/login/staff", (req, res) => {
  ...
  db.get(`SELECT * FROM staff_users WHERE username=? AND password=?`, [username, password], (err, row) => {
    if (row) { req.session.user = { role: "staff", fullName: row.full_name }; res.json({ user: req.session.user }); }
...
app.get("/api/availability", (req, res) => {
  ...
  db.all(`SELECT * FROM reservations WHERE status != 'cancelled'`, (err, rows) => {
    res.json(tables.map(t => ({
      ...t,
      isReserved: rows.some(r => {
        if (r.table_id !== t.id) return false;
        return (start < new Date(r.end_time) && end > new Date(r.start_time));
      })
    })));
  });
});
...
app.get("/api/reservations", (req, res) => {
  let sql = `SELECT * FROM reservations ORDER BY start_time DESC`;
  let params = [];
  if (req.session.user?.role === 'customer') {
    sql = `SELECT * FROM reservations WHERE customer_name = ? AND phone_number = ? ORDER BY start_time DESC`;
  ...
  db.get(`SELECT id FROM reservations WHERE customer_name=? AND phone_number=? AND start_time LIKE ? AND status != 'cancelled'`, 
    [req.session.user.fullName, req.session.user.phone, `${startDay}%`], (err, row) => {
      if(row && req.session.user.role === 'customer') {
          return res.status(400).json({ message: "Aynı gün içerisinde birden fazla rezervasyon yapılamaz." });
      }
...
// YENİ: PERSONEL İÇİN TAMAMEN SİLME FONKSİYONU
app.delete("/api/reservations/:id/purge", (req, res) => {
  if (req.session.user?.role !== 'staff') return res.status(403).json({ message: "Yetkisiz" });
  db.run(`DELETE FROM reservations WHERE id = ? AND status = 'cancelled'`, [req.params.id], () => res.json({ ok: true }));
});
